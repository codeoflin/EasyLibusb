...
#define USB_VENDOR_ID 0x0dd4
#define USB_PRODUCT_ID 0x0237
...
int ControlDevice(int vid,int pid,char requesttype,char request,char value,char index,char* buff,int len)
{
        ...
        rv = libusb_claim_interface(g_usb_handle,LIBUSB_CLASS_PRINTER);
        if(rv < 0) {
                rv = libusb_detach_kernel_driver(g_usb_handle,LIBUSB_CLASS_PRINTER);
                if(rv < 0) {
                        printf("*** libusb_detach_kernel_driver failed! rv%d\n",rv);
                        return -1;
                }
